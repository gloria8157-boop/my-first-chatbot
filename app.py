import streamlit as st
import os
import time
from openai import AzureOpenAI
from dotenv import load_dotenv

# 1. 환경 설정 및 클라이언트 초기화
load_dotenv() 

client = AzureOpenAI(
    azure_endpoint = os.getenv("AZURE_OAI_ENDPOINT"),
    api_key= os.getenv("AZURE_OAI_KEY"),
    # 파일 검색(file_ids) 기능 지원을 위해 안정적인 preview 버전을 사용
    api_version="2024-02-15-preview" 
)
# Assistant ID는 미리 생성하여 환경 변수 등에 저장해야 합니다.
ASSISTANT_ID = os.getenv("AZURE_ASSISTANT_ID") 

# Streamlit 세션 상태 초기화
if "thread_id" not in st.session_state:
    try:
        thread = client.beta.threads.create()
        st.session_state.thread_id = thread.id
        st.session_state.messages = []
    except Exception as e:
        st.error(f"Thread 생성 실패: {e}")
        st.stop()


# -------------------------------------------------------------
# 3. UI 및 파일 업로드
# -------------------------------------------------------------
st.title("💰 연말정산 서류 분석 챗봇 (Assistant API)")

for message in st.session_state.messages:
    with st.chat_message(message["role"]):
        st.markdown(message["content"])

# 파일 업로더
uploaded_file = st.file_uploader("분석할 연말정산 서류(PDF/이미지)를 업로드하세요.", 
                                 type=["pdf", "png", "jpg", "jpeg"], 
                                 key="tax_doc_uploader")


# -------------------------------------------------------------
# 4. 사용자 입력 및 Run 실행
# -------------------------------------------------------------
if prompt := st.chat_input("서류에 대한 질문이나 분석 요청을 입력하세요."):
    
    # 1. 사용자 메시지 화면 표시 및 세션 저장
    st.chat_message("user").markdown(prompt)
    st.session_state.messages.append({"role": "user", "content": prompt})
    
    file_id_to_analyze = None
    
    # 2. 파일 업로드 및 File ID 획득
    if uploaded_file is not None:
        try:
            with st.spinner(f"파일 업로드 및 인덱싱 중: {uploaded_file.name}"):
                file = client.files.create(
                    file=uploaded_file,
                    purpose="assistants"
                )
            file_id_to_analyze = file.id
            st.info(f"파일 업로드 완료. (File ID: {file_id_to_analyze})")
        
        except Exception as e:
            st.error(f"파일 업로드 실패: {e}")

    # 3. Thread에 메시지 추가 (파일 ID 연결)
    message_params = {
        "thread_id": st.session_state.thread_id,
        "role": "user",
        "content": prompt,
    }
    if file_id_to_analyze:
        # 파일이 성공적으로 업로드되었을 때만 file_ids 인자를 추가합니다.
        message_params["file_ids"] = [file_id_to_analyze]
    
    try:
        client.beta.threads.messages.create(**message_params)
    except Exception as e:
        st.error(f"메시지 추가 실패: {e}")
        st.stop()
    
    # 4. Run 실행 및 결과 대기
    with st.chat_message("assistant"):
        placeholder = st.empty()
        
        try:
            run = client.beta.threads.runs.create(
                thread_id=st.session_state.thread_id,
                assistant_id=ASSISTANT_ID
            )
        except Exception as e:
            placeholder.error(f"Assistant Run 실행 실패: {e}")
            st.stop()
            
        
        # Run 상태 대기
        with st.spinner("AI가 서류를 분석하고 답변을 생성 중입니다..."):
            while run.status in ['queued', 'in_progress', 'cancelling']:
                time.sleep(1)
                run = client.beta.threads.runs.retrieve(
                    thread_id=st.session_state.thread_id,
                    run_id=run.id
                )
        
        assistant_reply = f"오류 발생 또는 Run 상태: {run.status}"
        
        if run.status == 'completed':
            messages = client.beta.threads.messages.list(
                thread_id=st.session_state.thread_id,
                order='desc',
                limit=1
            )
            assistant_reply = messages.data[0].content[0].text.value
            
        # 5. 응답 출력 및 저장
        placeholder.markdown(assistant_reply)
        st.session_state.messages.append({"role": "assistant", "content": assistant_reply})
        st.session_state.messages.append({"role": "assistant", "content": assistant_reply})



